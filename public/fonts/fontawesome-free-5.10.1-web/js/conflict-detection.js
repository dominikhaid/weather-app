...
(function (global, factory) {
  ...
  function conflictDetection() {
    ...
    if (testCount === 0) {
      console.debug('%cAll Good!', 'color: green; font-size: large');
      console.debug(
        "We didn't find anything that needs testing for conflicts. Ergo, no conflicts."
      );
    } else {
      console.debug('Testing '.concat(testCount, ' possible conflicts.'));
      console.debug(
      ...
      pollUntil({
        ...
        .catch(function (e) {
          if (e === 'timeout') {
            ...
            console.debug('Whoops! We hit an error:', e);
            console.debug("Here's what we'd found up until that error:");
            setDoneResults({
              nodesTested: nodesTested,
              nodesFound: nodesFound,
            });
